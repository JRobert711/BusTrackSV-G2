...
import { Search, Filter, Star, Bus as BusIcon } from 'lucide-react';
import { Input } from './ui/input';
import { Button } from './ui/button';
import { Badge } from './ui/badge';
import { BusCard } from './BusCard';

function BusList({ buses, selectedBus, onBusSelect, onToggleFavorite }) {
      ...
      <div className="p-4 border-b">
        <div className="flex items-center gap-2 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Flota de Buses</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 mb-3">
          <select 
            ...
            className="flex-1 h-8 px-3 py-1 text-sm border border-gray-300 rounded-md bg-white"
          >
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="flex justify-between items-center text-sm">
            ...
            <Badge variant="secondary">{counts.favorites}</Badge>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span className="text-muted-foreground">En mov:</span>
            <Badge className="bg-green-100 text-green-800">{counts.moving}</Badge>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span className="text-muted-foreground">Parados:</span>
            <Badge className="bg-yellow-100 text-yellow-800">{counts.parked}</Badge>
          </div>
        </div>
      </div>
          ...
          {filteredBuses.length === 0 ? (
            ...
            filteredBuses.map((bus) => (
              <BusCard
                ...
                onToggleFavorite={() => onToggleFavorite(bus.id)}
              />
            ))
          )}
        </div>
